...
begin
  ...
  // Пускай таблицы будут одинаковыми - так легче будет отображать елементы
  ...
  begin
    ...
    StringGrid2.Width:= 280;
    StringGrid2.Height:= 200;
...
begin
  ...
  begin
    ...
    {Добавляем СТОЛБЕЦ со Средними арифметическими числами каждой строки в StringGrid1}
    ...
    {Защита}
    {
    {Добавляем СТРОКУ со Средними арифметическими числами каждой строки в StringGrid1}
    // Опять заметьте внутри with необязательно писать StringGrid1.RowCount
    RowCount := RowCount + 1;

    // Задаём размер этого столбца
    // Тк в StringGrid строки нумеруются с 0, то индекс последней строки
    // RowCount-1 - как раз наш добавленный столбец
    RowHeights[RowCount-1] := 50;

    // Тогда надо увеличить и высоту всей таблицы
    height := height + 50;

    // обходим столбцы матрицы
    for j:=1 to m do
    begin
      // Находим Среднее арифметическое чисел в i-ой строке
      ave := FindAveCol(a, n, j);
      // В StringGrid можно записать только строки
      // 5 - сколько полей выделяется на всё число
      // 2 - сколько значащих цифр после запятой
      str(ave:5:2, ave_str);
      // Записываем СРЕДНЕЕ каждого столбца таблицы в специально подготовленнюу нами
      // строку - последняя строка
      // RowCount-1 - индекс последней строки
      // j-1 - так как цикл перебирает с 1, а в StringGrid нумератция с нуля
      Cells[j-1,RowCount-1] := ave_str;
    end;
    }

    // Получаем число, от пользователя, чтобы понять
    ...
    begin
      ...
      // Здесь лучше не вызывать ещё раз FindAveRow
      // Мы же сохранили СРЕДНЕЕ для каждой строки в отдельный столбец
      // Вот и Считываем из этого столбца СРЕДНИЕ
    ...
    {Защита}
    {
    // теперь добавляем СТОЛБЦЫ в StringGrid2
    c := 0;
    for j:=1 to m do
    begin
      // Получаем Среднее для j-ого столюца
      // Здесь лучше не вызывать ещё раз FindAveCol
      // Мы же сохранили СРЕДНЕЕ для каждого столбца в отдельную строку
      // Вот и Считываем из неё СРЕДНИЕ
      // Но нужно привести их к целому типу
      ave := StrToFloat(Cells[j-1, RowCount-1]);
      // Если СРЕДНЕЕ больше K - то этот столбец должна быть в StringGrid2
      if (ave > k) then
        begin
          c := c+1; // увеличиваем счётчик добавляемых столбцов
          // Выводим с-ый столбик в StringGrid2
          for i:=1 to n do
            // Здесь Cells - относится к StringGrid1 из-за with
            // Для StringGrid2 нужно указывать полное имя через точку
            StringGrid2.Cells[c-1, i-1] := Cells[j-1, i-1];
         end;
     end;
     }
